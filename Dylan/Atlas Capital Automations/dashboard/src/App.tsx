 ...
 *
 * This dashboard implements a modern admin layout inspired by popular
 * open‑source templates like TailAdmin.  A collapsible sidebar on the
 * left contains navigation items for each feature (Single Score, Batch
 * Score, Explain, Workflows, Code Runner), while the top bar shows
 * branding and a hamburger button on small screens.  The layout is
 * responsive: on medium and larger screens the sidebar remains fixed,
 * whereas on small screens it slides in from the left when toggled.  An
 * overlay darkens the content when the sidebar is open on mobile,
 * helping users focus on navigation.
 */
export default function App() {
  ...
  // Define navigation items with icons.  We inline the icons here to avoid
  // re‑rendering them on every update.  Keys correspond to the tab values.
  ...
  // Utility to render the current panel.  Keeping this logic in a helper
  // function makes the JSX easier to read.
  const renderContent = () => {
    switch (currentTab) {
      case 'score':
        return <RiskScore />
