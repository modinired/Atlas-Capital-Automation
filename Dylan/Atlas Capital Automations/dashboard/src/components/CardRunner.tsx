...
// CardRunner component allows users to select a pre‑defined Card workflow and
// supply its inputs as JSON.  On submission it calls the MCP API and
// displays the returned dossier and execution context.  Users can also
// customise inputs to create new workflows.
const CardRunner: React.FC = () => {
  ...
  const loadTemplate = (selected: string) => {
    // Provide example templates for each known Card
    if (selected === 'QBR') {
    ...
    } else if (selected === 'IncidentPostmortem' || selected === 'IPM') {
      setInputText(JSON.stringify({
